...
# restore_model code original version...
## TypeError: float() argument must be a string or a real number, not 'dict' occurs :: return ops.EagerTensor(value, ctx.device_name, dtype)
# 
"""
def restore_model(model, layer_type):
    logging.info("Restore pre-trained weights")
    # download weights
    maybe_download_and_extract(model_saved_name[layer_type], '/home/oshadi/SISR-Final_Year_Project/envs/SISR-Project/State-of-art-models/SRGAN-master/model', model_urls[layer_type])
    weights = []
    if layer_type == 'vgg16':
        ...
        npz = np.load(os.path.join('/home/oshadi/SISR-Final_Year_Project/envs/SISR-Project/State-of-art-models/SRGAN-master/model', model_saved_name[layer_type]), allow_pickle=True, encoding='latin1').items()
        
        # npz = np.load(os.path.join('/home/oshadi/SISR-Final_Year_Project/envs/SISR-Project/State-of-art-models/SRGAN-master/model', model_saved_name[layer_type]), allow_pickle=True, encoding='latin1')
        # get weight list
        '''
        for val in sorted(npz):
        ...
        ''' 
        for val in sorted(npz):
            if np.ndim(val[1]) > 0:  # check if val[1] is not a scalar
                logging.info("  Loading %s in %s" % (str(val[1][0].shape), val[0]))
                weights.extend(val[1])
            else:
                logging.info("  Loading scalar in %s" % val[0])
                weights.append(val[1])  # use append for scalars
            if len(model.all_weights) == len(weights):
                break
...
"""
"""
model_path = '/home/oshadi/SISR-Final_Year_Project/envs/SISR-Project/State-of-art-models/SRGAN-master/model'
def restore_model(model, layer_type):
    logging.info("Restore pre-trained weights")
    # download weights
    maybe_download_and_extract(model_saved_name[layer_type], model_path, model_urls[layer_type])
    weights = []
    if layer_type == 'vgg16':
        npz = np.load(os.path.join(model_path, model_saved_name[layer_type]), allow_pickle=True)
        # get weight list
        for val in sorted(npz.items()):
            logging.info("  Loading weights %s in %s" % (str(val[1].shape), val[0]))
            weights.append(val[1])
        ...
        npz = np.load(os.path.join(model_path, model_saved_name[layer_type]), allow_pickle=True, encoding='latin1')
        for val in sorted(npz.items()):
            if np.ndim(val[1]) > 0:  # check if val[1] is not a scalar
                logging.info("  Loading %s in %s" % (str(val[1][0].shape), val[0]))
                weights.extend(val[1])
    ...
    if tlx.BACKEND != 'tensorflow':
        for i in range(len(weights)):
            if len(weights[i].shape) == 4:
                weights[i] = np.transpose(weights[i], axes=[3, 2, 0, 1])
    # Check if weights is a dictionary and handle accordingly
    if isinstance(weights, dict):
        weights = [weights[key] for key in sorted(weights.keys())]
    assign_weights(weights, model)
    del weights
...
## Check....
# I have to do these modification in vgg script
'''
Inspect the vgg19 Key: The code now retrieves the contents of the vgg19 key and checks if it is a dictionary.
Iterate Through Dictionary Items: If the vgg19 key contains a dictionary, the code iterates through the items and appends the arrays to the weights list.
Skip Non-Array Weights: The code logs a warning and skips any non-array weights to avoid errors.
Error I got::
1. TypeError: float() argument must be a string or a real number, not 'dict'
2. IndexError: too many indices for array: array is 0-dimensional, but 1 were indexed
cause :: " logging.info("  Loading %s in %s" % (str(val[1][0].shape), val[0])) "

3. run 
python /home/oshadi/SISR-Final_Year_Project/envs/SISR-Project/State-of-art-models/SRGAN-master/version_insights.py
ensure newly downloaded vgg version is matches with the code build by here.
...
def restore_model(model, layer_type):
    ...
    if layer_type == 'vgg16':
        ...
        if isinstance(vgg19_data, dict):
            for key, val in sorted(vgg19_data.items()):
                if isinstance(val, np.ndarray):
                    logging.info("  Loading %s in %s" % (str(val.shape), key))
                    weights.append(val)
                else:
                    logging.warning("Skipping non-array weight: %s" % key)
                if len(model.all_weights) == len(weights):
                    break
        else:
            logging.warning("Unexpected data structure in vgg19 key")
    if tlx.BACKEND != 'tensorflow':
        for i in range(len(weights)):
            if len(weights[i].shape) == 4:
                weights[i] = np.transpose(weights[i], axes=[3, 2, 0, 1])
