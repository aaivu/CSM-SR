...
from modules import VAE_denoise, VGGFeatureExtractor, ConvBlock, DeconvBlock, ResnetBlock
...
parser.add_argument('--pretrained_sr', default='/home/oshadi/SISR-Final_Year_Project/envs/SISR-Project/State-of-art-models/dSRVAE-master/dSRVAE-master/VAE_epoch_160.pth', help='sr pretrained base model')
...
def train(epoch):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        # Print shape of HR_feat
        # print(f"HR_feat shape: {HR_feat.shape}")

        Denoise_LR, KL = model(HR_feat, input)
        # Print shape of Denoise_LR
        # print(f"Denoise_LR shape: {Denoise_LR.shape}")
...
# training_data_loader = DataLoader(dataset=train_set, num_workers=opt.threads, batch_size=opt.batchSize, shuffle=True) ## original
# Modified...
...
model = VAE_denoise(input_dim=3, dim=32, feat_size=8, z_dim=512, prior='standard') # input IS b&w 
# model = VAE_denoise(input_dim=1, dim=32, feat_size=8, z_dim=512, prior='standard')
