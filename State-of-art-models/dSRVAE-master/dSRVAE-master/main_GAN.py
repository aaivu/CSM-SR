...
from modules import VAE_denoise_vali, discriminator, VAE_SR, VGGFeatureExtractor, ConvBlock, DeconvBlock, ResnetBlock
...
parser.add_argument('--pretrained_iter', type=int, default=5000, help='number of epochs to train for')
...
parser.add_argument('--data_dir', type=str, default='/home/oshadi/SISR-Final_Year_Project/envs/SISR-Project/data/DIV2K')
parser.add_argument('--data_augmentation', type=bool, default=False)
parser.add_argument('--patch_size', type=int, default=256, help='Size of cropped LR image')
parser.add_argument('--pretrained_sr', default='/home/oshadi/SISR-Final_Year_Project/envs/SISR-Project/State-of-art-models/dSRVAE-master/dSRVAE-master/models/VAE_epoch_5000.pth', help='sr pretrained base model')
...
parser.add_argument('--save_folder', default='/home/oshadi/SISR-Final_Year_Project/envs/SISR-Project/State-of-art-models/dSRVAE-master/dSRVAE-master/models/', help='Location to save checkpoint models')
parser.add_argument('--log_folder', default='/home/oshadi/SISR-Final_Year_Project/envs/SISR-Project/State-of-art-models/dSRVAE-master/dSRVAE-master/logs/', help='Location to save checkpoint models')
...
# training_data_loader = DataLoader(dataset=train_set, num_workers=opt.threads, batch_size=opt.batchSize, shuffle=True) ## Original. Below compatible with the denoiser
...
model_denoiser = os.path.join(opt.save_folder + 'VAE_epoch_5000.pth')
# denoiser.load_state_dict(torch.load(model_denoiser, map_location=lambda storage, loc: storage)) ## Original
from collections import OrderedDict
state_dict = torch.load(model_denoiser, map_location=lambda storage, loc: storage)
new_state_dict = OrderedDict()
for k, v in state_dict.items():
    if k.startswith('module.'):
        name = k[7:]  # remove `module.` if it exists
    else:
        name = 'module.' + k  # add `module.` if it doesn't exist
...
if opt.pretrained:
    ...
    #     D.load_state_dict(torch.load(model_D, map_location=lambda storage, loc: storage), strict=False)
