...
def get_patch(img_in, img_tar, img_ref, patch_size, scale, ix=-1, iy=-1):
    ...
    # tp = patch_mult * patch_size  ## Original..
    tp = patch_size  # Use the patch_size directly
    ip = tp // scale

    # Debugging statements
    print(f"Input image size: {ih}x{iw}")
    print(f"Patch size: {tp}")
    print(f"Upscale factor: {scale}")
    print(f"Calculated patch size: {ip}")

    print(f"iw, tp: {iw}x{tp}")
    if ix == -1:
    ...
    print(f"Output image size: {iy}x{ix}")
...
class DatasetFromFolder(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # ref = modcrop(ref, 16)
        ref_index = (len(self.hr_image_filenames) - index) % len(self.hr_image_filenames)
        ref = load_img(self.hr_image_filenames[ref_index])
        ref = modcrop(ref, 16)
