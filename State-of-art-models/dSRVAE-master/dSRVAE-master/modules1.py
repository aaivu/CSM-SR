...
class VAE_denoise(nn.Module):
    ...
    def reparameterize(self, mu, logvar, flag=0):
        if flag == 0:
            ...
            eps = torch.cuda.FloatTensor(std.size()).normal_()  ## original
            # eps = torch.randn(std.size(), device='cuda')
    ...
    def reparameterize(self, mu, logvar, flag=0):
        ...
        if flag == 0:
            ...
            eps = torch.from_numpy(np.random.normal(0, 0.05, size=(std.size(0), 1, std.size(2), std.size(3)))).float().cuda()
            eps = eps.repeat(1, 3, 1, 1)
        z = eps.mul(std).add_(mu)
        return z
    '''
    ...
    def decode(self, LR, z_q):
        ...
        # Print shape of LR_feat
        # print(f"LR_feat shape: {LR_feat.shape}")

        dec_feat = self.VAE_decoder(z_q)
        dec_feat = dec_feat.view(dec_feat.size(0), -1, self.feat_size, self.feat_size)
        # Print shape of dec_feat
        # print(f"dec_feat shape: {dec_feat.shape}")

        mu_feat = self.decoder(dec_feat)
        # Print shape of mu_feat
        # print(f"mu_feat shape: {mu_feat.shape}")
    ...
    def forward(self, HR_feat, LR):
        ...
        # Print shapes of z_q_mu and z_q_logvar
        # print(f"z_q_mu shape: {z_q_mu.shape}")
        # print(f"z_q_logvar shape: {z_q_logvar.shape}")
        # reparameterize
...
class VAE_denoise_vali(nn.Module):
    def __init__(self, input_dim, dim, feat_size, z_dim, prior):
        ...
        #     DeconvBlock(4 * dim, 2 * dim, 6, 4, 1),
        #     ConvBlock(2 * dim, dim, 3, 1, 1),
        #     nn.Upsample(scale_factor=0.25, mode='bicubic'),  # Add this layer to match the dimensions
        #     nn.Conv2d(dim, dim, 3, 1, 1)  # Add this layer to ensure the output dimensions match LR_feat
        # )
        ...
        self.decoder = nn.Sequential(
            ...
            DeconvBlock(4 * dim, 2 * dim, 6, 4, 1),
            ConvBlock(2 * dim, dim, 3, 1, 1),
            nn.Conv2d(dim, dim, 3, 2, 1),  # Adjust dimensions          ## Comment when testing, Uncomment training...
            # nn.Conv2d(dim, dim, 3, 2, 1),  # Further adjust dimensions  ## Comment when testing, Uncomment training...
            # nn.Conv2d(dim, dim, 3, 1, 1),  # Further adjust dimensions
        )   
...
class discriminator(nn.Module):
    def __init__(self, num_channels, base_filter, image_size):
        ...
        self.classifier = nn.Sequential(
            # nn.Linear(512 * 4 * 4, 100), ## Original
            nn.Linear(512 , 100),
            nn.ReLU(),
            # nn.BatchNorm1d(100),
            nn.Linear(100, 1),
    ...
    def forward(self, x):
        ...
        # print(f"conv_blocks output shape: {out.shape}")  # Add this line
